...
| [MRK-005](NORMATIVE.md#MRK-005) | :white_check_mark: | Proof verification | `tests/mrk_005_tests.rs`: `SparseMerkleProof::verify`, `verify_coin_proof` (`src/merkle/proof.rs`, SPEC / IMPLEMENTATION_ORDER name), inclusion + exclusion + empty-tree vs `empty_hash(SMT_HEIGHT)`, tampered sibling/leaf + wrong root, snapshot semantics (old root still valid), `verify_coin_proof` ≡ `verify`; test-local `reference_verify` duplicates the walk for cross-check. |
