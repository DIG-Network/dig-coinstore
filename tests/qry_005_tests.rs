...
fn vv_req_qry_005_include_spent_true() {
    ...
    let results = store.get_coin_records_by_names(true, &[genesis_coin.coin_id(), new_coin.coin_id()], 0, u64::MAX).unwrap();
...
fn vv_req_qry_005_include_spent_false() {
    let (store, _dir, genesis_coin, new_coin) = setup();
    let results = store.get_coin_records_by_names(false, &[genesis_coin.coin_id(), new_coin.coin_id()], 0, u64::MAX).unwrap();
...
fn vv_req_qry_005_height_range() {
    ...
    let results = store.get_coin_records_by_names(true, &[genesis_coin.coin_id(), new_coin.coin_id()], 1, 1).unwrap();
...
fn vv_req_qry_005_empty_names() {
    ...
    let results = store.get_coin_records_by_names(true, &[], 0, u64::MAX).unwrap();
