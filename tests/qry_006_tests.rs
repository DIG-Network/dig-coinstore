...
fn vv_req_qry_006_coin_states_by_puzzle_hashes() {
    let (store, _dir, _genesis, _new_coin) = setup();
    let ph = helpers::test_hash(11); // new_coin puzzle hash
    let states = store
        .get_coin_states_by_puzzle_hashes(true, &[ph], 0, 100)
        .unwrap();
...
fn vv_req_qry_006_spent_coin_state_has_spent_height() {
    ...
    let states = store
        .get_coin_states_by_ids(true, &[genesis_coin.coin_id()], 0, u64::MAX, 100)
        .unwrap();
    assert_eq!(states.len(), 1);
    assert!(
        states[0].spent_height.is_some(),
        "Spent coin must have spent_height"
    );
