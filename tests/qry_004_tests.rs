...
fn setup() -> (CoinStore, tempfile::TempDir) {
    ...
    store
        .init_genesis(vec![(c1, false)], 1_700_000_000)
        .unwrap();
    (store, dir)
}
...
fn vv_req_qry_004_by_parent_id() {
    ...
    let results = store
        .get_coin_records_by_parent_ids(true, &[parent], 0, u64::MAX)
        .unwrap();
...
fn vv_req_qry_004_no_match() {
    ...
    let results = store
        .get_coin_records_by_parent_ids(true, &[unknown], 0, u64::MAX)
        .unwrap();
...
fn vv_req_qry_004_include_spent_false() {
    ...
    let results = store
        .get_coin_records_by_parent_ids(false, &[parent], 0, u64::MAX)
        .unwrap();
...
fn vv_req_qry_004_multiple_parents() {
    ...
    let results = store
        .get_coin_records_by_parent_ids(true, &[parent1, parent2], 0, u64::MAX)
        .unwrap();
