...
fn vv_req_qry_011_large_coin_ids_batch() {
    ...
    let ids: Vec<Bytes32> = (0..100u8).map(|i| helpers::test_hash(i)).collect();
...
fn vv_req_qry_011_large_puzzle_hash_batch() {
    ...
    let phs: Vec<Bytes32> = (0..100u8).map(|i| helpers::test_hash(i)).collect();
    let results = store.get_coin_records_by_puzzle_hashes(true, &phs, 0, u64::MAX).unwrap();
    assert!(results.is_empty());
}
...
fn vv_req_qry_011_large_names_batch() {
    ...
    let names: Vec<Bytes32> = (0..100u8).map(|i| helpers::test_hash(i)).collect();
    let results = store.get_coin_records_by_names(true, &names, 0, u64::MAX).unwrap();
    assert!(results.is_empty());
}
...
fn vv_req_qry_011_large_parent_ids_batch() {
    ...
    let parents: Vec<Bytes32> = (0..100u8).map(|i| helpers::test_hash(i)).collect();
    let results = store.get_coin_records_by_parent_ids(true, &parents, 0, u64::MAX).unwrap();
    assert!(results.is_empty());
}
