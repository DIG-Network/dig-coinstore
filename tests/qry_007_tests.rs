...
use dig_coinstore::{coin_store::CoinStore, Bytes32, CoinAddition, CoinStateFilters};

fn make_filters(include_spent: bool, include_unspent: bool, include_hinted: bool, min_amount: u64) -> CoinStateFilters {
...
fn vv_req_qry_007_basic() {
    ...
    let (states, next) = store.batch_coin_states_by_puzzle_hashes(&[ph], 0, filters, 100).unwrap();
...
fn vv_req_qry_007_exclude_spent() {
    ...
    let (states, _) = store.batch_coin_states_by_puzzle_hashes(&[ph], 0, filters, 100).unwrap();
...
fn vv_req_qry_007_batch_too_large() {
    ...
    let many_phs: Vec<Bytes32> = (0..991u16).map(|i| {
        let mut b = [0u8; 32];
    ...
    assert!(matches!(result, Err(dig_coinstore::CoinStoreError::PuzzleHashBatchTooLarge { .. })));
}
...
fn vv_req_qry_007_pagination() {
    let (store, _dir, ph) = setup();
    let filters = make_filters(true, true, false, 0);
    let (states, next) = store.batch_coin_states_by_puzzle_hashes(&[ph], 0, filters, 1).unwrap();
